...
public class CustomerTest extends BaseTest {
    ...
    @Test(description = "TC_CU_001: Verify that customer should be able to log in successfully")
    ...
    @Test(description = "TC_CU_002: Verify that customer name should be displayed on dashboard")
    @Story("Customer Login")
    ...
    public void verifyCustomerNameDisplayedAfterLogin() {
        ...
        Assert.assertTrue(displayedName.contains("Harry"),
                "Customer name should be displayed on the dashboard.");
    }

    @Test(description = "TC_CU_003: Verify that customer without account cannot access banking features")
    @Story("Customer Login")
    ...
    @Test(description = "TC_CU_004: Verify that customer should be able to deposit a valid amount")
    @Story("Deposit Funds")
    @Severity(SeverityLevel.CRITICAL)
    ...
    public void verifyDepositValidAmount() {
        ...
        Assert.assertTrue(accountPage.isDepositSuccessful(),
        ...
        Assert.assertEquals(Integer.parseInt(balanceAfter), expectedBalance,
                "Account balance should be updated after deposit.");
    }

    @Test(description = "TC_CU_005: Verify that account balance should update after deposit")
    @Story("Deposit Funds")
    @Severity(SeverityLevel.CRITICAL)
    ...
    public void verifyAccountBalanceUpdatesAfterDeposit() {
        ...
        Assert.assertEquals(Integer.parseInt(balanceAfter),
                initialBalance + Integer.parseInt(TestData.DEPOSIT_AMOUNT_SMALL),
                "Balance should increase by the deposited amount.");
    }

    @Test(description = "TC_CU_006: Verify that customer should not be able to deposit zero amount")
    ...
    public void verifyDepositZeroAmount() {
        ...
        Assert.assertFalse(isSuccess, "System should not allow deposit of zero amount.");
    }

    @Test(description = "TC_CU_007: Verify that customer should not be able to deposit negative amount")
    @Story("Deposit Funds - Validation")
    @Severity(SeverityLevel.NORMAL)
    ...
    public void verifyDepositNegativeAmount() {
        ...
        boolean isSuccess = accountPage.isDepositSuccessful();
        Assert.assertFalse(isSuccess, "System should not allow deposit of a negative amount.");
    }

    // ─── Withdrawal Tests ─────────────────────────────────────────────────────

    @Test(description = "TC_CU_008: Verify that customer should be able to withdraw a valid amount")
    ...
    public void verifyWithdrawValidAmount() {
        ...
        Assert.assertEquals(Integer.parseInt(balanceAfterWithdrawal), expectedBalance,
                "Balance should decrease by the withdrawn amount.");
    }

    @Test(description = "TC_CU_009: Verify that system should reject withdrawal exceeding account balance")
    ...
    public void verifyWithdrawExceedingBalance() {
        ...
        Assert.assertTrue(accountPage.isTransactionFailed(),
                "System should reject withdrawal amount exceeding account balance.");
    }

    @Test(description = "TC_CU_010: Verify that system should reject withdrawal of zero amount")
    @Story("Withdraw Money - Validation")
    @Severity(SeverityLevel.NORMAL)
    ...
    @Test(description = "TC_CU_011: Verify that balance should remain unchanged after failed withdrawal")
    @Story("Withdraw Money - Validation")
    @Severity(SeverityLevel.NORMAL)
    ...
    @Test(description = "TC_CU_012: Verify that transactions tab should show list of transactions")
    ...
    public void verifyViewTransactionsTab() {
        ...
        Assert.assertTrue(transactionCount >= 1,
                "Transaction list should show at least one transaction after a deposit.");
    }

    @Test(description = "TC_CU_013: Verify that transactions should be recorded after deposit")
    @Story("View Transactions")
    @Severity(SeverityLevel.NORMAL)
    ...
    public void verifyDepositAppearsInTransactions() {
        ...
        Assert.assertTrue(accountPage.getTransactionCount() > 0,
                "Deposit transaction should appear in the transaction history.");
    }

    @Test(description = "TC_CU_014: Verify that transactions should be recorded after withdrawal")
    @Story("View Transactions")
    @Severity(SeverityLevel.NORMAL)
    ...
    public void verifyWithdrawalAppearsInTransactions() {
        ...
        Assert.assertTrue(accountPage.getTransactionCount() >= 2,
                "Both deposit and withdrawal should appear in transaction history.");
    }

    // ─── Transaction Security Tests ───────────────────────────────────────────

    @Test(description = "TC_CU_015: Verify that customer should not be able to alter transaction history via Reset")
    @Story("Transaction Security")
    @Severity(SeverityLevel.CRITICAL)
    @Description("Verify that transaction history cannot be manipulated by the customer - Reset should only filter display, not delete actual records.")
    public void verifyTransactionHistoryCannotBeAltered() {
        accountPage.deposit(TestData.DEPOSIT_AMOUNT_VALID);
        accountPage.clickTransactionsTab();

        boolean resetDisplayed = accountPage.isResetButtonDisplayed();
        Assert.assertTrue(resetDisplayed,
                "Reset button for display filtering should be present but not able to alter backend records.");
    }

    @Test(description = "TC_CU_016: Verify that customer should be able to log out successfully")
    @Story("Customer Login")
    @Severity(SeverityLevel.NORMAL)
